...
/**
 * Validates the Gemini API key directly from the browser,
 * trying multiple model versions in order of preference.
 */
export async function validateApiKeyDirect(apiKey: string): Promise<boolean> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        try {
          ...
          parsedError = parsed.error?.message || errText;
        } catch {
          parsedError = errText;
        }
        lastError = new Error(`HTTP ${response.status}: ${parsedError}`);
      }
    } catch (err: any) {
      lastError = err;
    }
  ...
  throw lastError || new Error("API Key validation failed");
}

/**
 * Generates the marketing plan directly from the browser,
 * trying multiple model versions in order of preference if server-side proxy is unavailable.
 */
export async function generatePlanDirect(answers: SurveyAnswers, apiKey: string): Promise<string> {
  const {
    step0,
    step1,
    step2,
...
- 각 채널별로 확인하면 좋은 핵심 지표 (저장수/공유수, 완주율, 프로필 클릭률, 유입 전환율 등) 1~2개씩 구체적으로 제안`;
  ...
  throw lastError || new Error("제안서 자동 생성에 실패했습니다.");
}
